...
SUPABASE_URL = os.getenv("SUPABASE_URL", "https://ujseuwywmnmoaixxezfd.supabase.co")
SUPABASE_KEY = os.getenv("SUPABASE_KEY", "eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJpc3MiOiJzdXBhYmFzZSIsInJlZiI6InVqc2V1d3l3bW5tb2FpeHhlemZkIiwicm9sZSI6ImFub24iLCJpYXQiOjE3MTU4NzQ0NDIsImV4cCI6MjAzMTQ1MDQ0Mn0.o86ZacwpiqE0FDZKifW3QpHMXqZiV8GHt6oRdDkyP_4")
SUPABASE_ID = "ujseuwywmnmoaixxezfd"
...
def authCallback():
    if request.method == 'POST':
        ...
        if not data:
            logging.error("No data received in the POST request.")
        ...
        if not accessToken or not refreshToken:
            logging.error("Missing accessToken or refreshToken in the request data.")
            return "Missing accessToken or refreshToken", 400

        logging.info(f"AccessToken: {accessToken}, RefreshToken: {refreshToken}")
        ...
        try:
            ...
            logging.info(f"User session set for email: {sessionData.user.email}")
        except Exception as E:
            logging.exception("Cannot set session data. ERROR: " + str(E))
            return "Cannot fetch data from the database.", 407

        try:
            data, count = client.table('User').select('email', count='exact').eq('email', str(sessionData.user.email)).execute()
            logging.info(f"User data fetched: {data}, count: {count}")
        except Exception as E:
            logging.exception("Cannot fetch data from the database. ERROR: " + str(E))
            return "Cannot fetch data from the database.", 408

        if count[1] == 0:
            logging.info("Account creation process is proceeding")
            return "Account creation process is proceeding", 202
        else:
            logging.info("Account already found")
            return "Account already found", 409

    # Handling GET request
    elif request.method == 'GET':
        logging.info("GET request received, rendering waiting_page.html")
...
def send_email(receiverEmail, payload):
    ...
    if "type" in payload and (payload["type"] == "UPDATE" or payload["type"] == "UPSERT"):
        ...
        Dear User,
        <br><br>
        We are sad to inform you that there were some changes in the flight course.
        <br><br>
        ...
        <p>{("<i>Origin Location: </i> " + str(payload['record']['origin_location']) if payload['record']['isArriving'] is True else "<i>Destination Location: </i>" + str(payload['record']['destination_location']))}</p>
    ...
    try:
        smtpObj.ehlo()
        smtpObj.starttls()
        smtpObj.login('Haninebay@gmail.com', "Hanouna1234@")
        smtpObj.sendmail('Haninebay@gmail.com', receiverEmail, message.as_string())
        smtpObj.quit()
    except Exception as E:
